...
def get_prices(ticker: str, start_date: str, end_date: str) -> tuple[list[Price], str | None]:
    ...
    # Process newly_fetched_prices (corresponds to 'prices' in the prompt's logic block)
    # Initial check for empty newly_fetched_prices and caching are handled before this block.

    filtered_prices = []
    if start_date == end_date: # Single-day request (e.g., for latest price)
        logging.info(f"get_prices: Single-day request for {ticker} on {start_date}. Applying specific filtering.")
        try:
            # Ensure p.time is valid before sorting. Add error handling or filter out invalid times if necessary.
            # For simplicity, assuming p.time is mostly valid "YYYY-MM-DD HH:MM:SS+/-TZ"
            # or just "YYYY-MM-DD" if that's what yfinance_transformers guarantees for Price.time
            # If Price.time can be None or malformed, add checks. Let's assume it's a string "YYYY-MM-DD...".
            sorted_prices = sorted(newly_fetched_prices, key=lambda p: p.time or "", reverse=True)
        except TypeError as e: # Catch if p.time is None for example
            logging.error(f"Error sorting prices for {ticker} due to invalid time data: {e}. Prices: {newly_fetched_prices}")
            return [], f"Failed to sort price data for {ticker} due to invalid time entries."

        match_found = False
        for p in sorted_prices:
            if p.time and p.time[:10] <= start_date: # Found the latest price on or before the requested date
                filtered_prices = [p]
                logging.info(f"For single-day request {start_date} for {ticker}, using price from {p.time[:10]}.")
                match_found = True
                break 
        if not match_found:
             logging.warning(f"For single-day request {start_date} for {ticker}, no price found on or before this date in fetched window ({len(newly_fetched_prices)} records).")
             # Error message will be generated by the subsequent 'if not filtered_prices:' block
    else: # Range request
        filtered_prices = [
            p for p in newly_fetched_prices
            if p.time and start_date <= p.time[:10] <= end_date
        ]

    if not filtered_prices:
        # Generate a more informative error message based on whether it was a single day or range
        if start_date == end_date:
            err_msg = f"No data found for {ticker} on or before {start_date} after filtering {len(newly_fetched_prices)} fetched records."
        else:
            err_msg = f"No data found for {ticker} in range {start_date}-{end_date} after filtering {len(newly_fetched_prices)} fetched records."
        logging.warning(err_msg) # Ensure this is logging.warning
        return [], err_msg
    
    return filtered_prices, None
